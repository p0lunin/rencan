...
layout(set = 1, binding = 0) readonly buffer IndirectRays {
    LightningRay next_rays[];
};
layout(set = 1, binding = 1) writeonly buffer IndirectIntersections {
...
void main() {
    ...
    if (inter.is_intersect == 1 && model.material == MATERIAL_DIFFUSE) {
        ...
        Ray shadow_ray_for_global = make_shadow_ray_for_direction_light(inter);

        next_intersections[offset] = trace_first(shadow_ray_for_global);

        for (int i = 0; i + 1 < MAX_INDIRECT_RAYS && i < point_lights_count; i++) {
            PointLight light = point_lights[i];
            Ray ray = make_shadow_ray_for_point_light(inter, light);

            next_intersections[offset + i + 1] = trace_first(ray);
        }
    }
}