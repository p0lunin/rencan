...
use vulkano::command_buffer::{AutoCommandBuffer, CommandBuffer, AutoCommandBufferBuilder};
...
pub fn rays_from_vec(device: Arc<Device>, vec: Vec<Point4<f32>>) -> Arc<CpuAccessibleBuffer<[Point4<f32>]>> {
    CpuAccessibleBuffer::from_iter(
        device.clone(),
        BufferUsage::all(),
        false,
